...
-- 예: 데이터베이스에 사용자가 원하는 특정 데이터를 보여달라도 요청하는 것 
...
-- 삭제 전에는 항상 백업이 되어 있는지, 그리고 정말 삭제해야하는 대상이 맞는지 확인하는 습관 
...
/*
	2.3 데이터베이스 삽입 및 조회하기 
*/
-- QUIZ: 다시 mapdonalds DB생성 및 진입 
CREATE DATABASE mapdonalds;
USE mapdonalds;
...
-- 수백만, 수천만 건의 데이터가 들어있는 거대한 테이블의 구조를 변경하는 작업은 많은 시간과 시스템 자원을 소모
-- 작업 중에는 테이즐이 잠겨서 서비스가 일시적으로 멈출 수도 있음 
...
-- 컬럼 생략 가능? 가능하지만 테이블 정의에 맞게 모든 열에 순서대로 넣어야함 
...
-- 테이블의 모든 데이터 삭제 (테이블의 구조는 살리고 데이터만 삭제) 
...
-- Why? 조회 성능 저하, 가독성 저하, 네트워크 트랙픽 낭비 등
...
-- 에러 메세지 확인: 안전 모드에서는 KEY 없는 UPDATE/DELETE를 차단함
...
-- 특정(단일) 레코드 수정하기 
-- 빅맨 버거만 가격을 500원으로 수정 
UPDATE burgers
SET price = 500
WHERE name = '빅맨';  -- 특정 대상 변경시, 조건은 반드시 기본키를 사용
...
-- '슈비두밥 버거'가 단종 됐다면, 이를 위한 데이터 삭제 쿼리는? 
DELETE FROM burgers
WHERE name = '슈비두밥 버거';  -- 에러발생: 수정과 동일하게 조건은 반드시 기본키를 사용함
