...
-- 조인하기
-- 특정 사진에 달린 모든 댓글의 작성자 '닉네임'과 댓글 본문을 조회하는법? 
-- 필요요소: 댓글 / 사용자 
...
FROM comments
JOIN users 
ON comments.user_id = users.id; -- 조인 컬럼 (외래키) = 조인 컬럼(기본키)

-- 2) 조인 컬럼은 서로 자료형이 일치 
-- 일치해야 조인 가능
-- 예 숫자1과 문자열 '1'은 서로 조인 불가
-- 다만 DBMS마자 자동 형 변환이 될 수 있음
...
-- 컬럼명이 같은 경우 어느 테이블의 것인기 명시해야함 (그렇지 않은 경우 에러 발생) 
...
-- 7) 다양한 조인 유형 사용가능
-- 조인은 크게 내부조인(INNER JOIN)과 외부조인(OUTER JOIN)으로 나뉨
-- 다양한 결과 테이블 생성에 도움 
-- INNER 조인
-- LEFT 조인  (OUTER 조인)
-- RIGHT 조인 (OUTER 조인)
...
-- FROM comments ① ___c___
-- ② ____JOIN______ photos AS p ③ ____ON____ c.photo_id = p.id;
...
SELECT 컬러명1, 컬럼명2, ... 
FROM 테이블A
INNER JOIN 테이블 B ON 테이블A.조인_칼럼 = 테이블B.조인_칼럼;
-- 실무에서는 INNER 키워드를 생략

-- QUIZ: photos 테이블과 users테이블 INNER 조인 (INNER조인은 교집합을 보여주는 것이기에 붙이는 방향이 크게 상관없다.)
SELECT *
FROM photos p
