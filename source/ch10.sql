...
-- 1) 개념적 데이터 모델링
-- 현실 세계를 구성하는 주요 개체(엔티티, entity(테이블))와 그들 간 관계(relation)를 정의
...
-- (Ch10_erd2 이미지 참고)
-- ER 다이어그램 해석1
-- 사용자는 프로필을 가지지 않을 수 있고, 가진다면 최대 1개 가질 수 있다 (0~1) (선택적일대일)
-- 프로필은 사용자와 관계를 맺는데, 반드시 가져야 한다.(1~1) (필수적일대일)

-- ER 다이어그램 해석2
-- 사용자는 주문을 안 할 수도 있지만, 주문을 한다면 여러개의 주문을 할 수 있다. (0~N)
-- 주문은 특정 사용자를 참조할 수도 있고, 또는 참조 하지 않을 수도 있다. 즉, 비회원인 경우에도 주문 가능 (0~1)

-- Quiz
-- 2. 다음 빈칸에 들어갈 용어를 순서대로 고르시오.
-- ① ____ㄴ____: 비즈니스에서 중요 대상을 모형화한 것으로 데이터베이스로 구현했을 때 테이블이 됨
-- ② ____ㅁ____: 엔티티의 실제 데이터로, 테이블에 저장된 행 단위의 데이터
-- ③ ____ㄱ____: 엔티티의 특성이나 정보를 나타내는 항목으로 데이터베이스로 구현했을 때 칼럼이 됨
-- ④ ____ㄹ____: 엔티티의 한 인스턴스가 다른 엔티티의 몇 개 인스턴스와 연결되는지를 나타내는 개념
-- ⑤ ____ㄷ____: 엔티티 내에서 각 인스턴스를 고유하게 식별하는 속성 또는 속성의 조합
...
-- 테이블명_id는 조인할 떄 가독성이 좋고 어떤테이블의 PK인지 명확하게 알 수 있음

-- 2) 컬럼의 자료형 정의(DBSM에 맞게) 
-- 예: MySQL에서 지워하는 자료형 중 해당 컬럼을 가장 잘 저장할 수 있는 것으로 표시

-- 3) 고유키(+제약조건) 및 인덱스 지정
-- users 테이블의 email과 products 테이블의 name을 고유키로 지정 
--  ㄴ고유키는 UK로 표시
-- products 테이블의 name을 인덱스 처리 (고유키로 설정했기때문에 자동으로 인덱스가 설정됨) 
--  ㄴ상품명으로 검색 및 정렬할 때 성능을 높임
...
-- ① ____ㄹ____: 엔티티 간 관계가 필수인지 선택인지를 정의하는 개념
-- ② ____ㅁ____: 컬럼이 NULL 값을 가질 수 없게 하는 제약 조건
-- ③ ____ㄴ____: 컬럼에 입력되는 값이 특정 조건을 만족하도록 하는 제약 조건
-- ④ ____ㄷ___: 컬럼에 값이 입력되지 않을 때 기본으로 저장된 값을 설정하는 제약 조건
-- ⑤ ____ㄱ___: 데이터베이스에서 데이터의 검색 및 정렬 성능을 빠르게 해 주는 데이터 구조로 테이블의 특정 컬럼에 생성해 활용
...
-- ① ____ㅁ____: 데이터의 중복을 최소화하고 데이터 무결성을 보장하기 위해 데이터를 구조화 하는 작업으로, 
--               잘못된 데이터 모델링을 사전에 검증하고 개선하는 것
-- ② ____ㄱ____: 테이블의 컬럼을 더 이상 분해할 수 없도록 나누는 것
-- ③ ____ㄷ____: 일반 속성이 기본키의 전체 속성에 의해 결정되는 것
-- ④ ____ㄹ____: 일반 속성이 기본키의 일부 속성에 의해 결정되는 것
-- ⑤ ____ㄴ____: 일반 속성이 다른 일반 속성에 의해 값이 결정되는 것
