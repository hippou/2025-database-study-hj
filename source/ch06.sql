...
-- 1부터 시작하여 데이터가 추가 될때마다 1씩 자동으로 증가하는 정수값(INETEGER)을 기본키로 사용하는 경우가 가장 흔함

-- 외래키(FK, Foreign Key)
-- 다른 테이블의 기본키를 가리키는 컬럼
-- 두 테이블을 관계로 묶어 연결하는 역할
-- 예: 고객 테이블(PK: customer_id,참조 당하는 쪽,부모 테이블) <--(참조)--주문 테이블(FK: cutomer_id, 참조하는 쪽,자식 테이블) 
...
-- 제약 조건 위반시 테스트 
-- 사용자면(NOT NUll)에 NULL값을 입력 시도 
INSERT INTO users(email,name)
VALUES ('geobleo@naver.com', null);
-- Error Code: 1048. Column 'name' cannot be null

-- 이메일 (UNIQUE)에 중복 값을 입력 시도 
INSERT INTO users (email,name)
VALUES ('geobleo@naver.com', '김');

INSERT INTO users (email,name)
VALUES ('geobleo@naver.com', '재');
-- Error Code: 1062. Duplicate entry 'geobleo@naver.com' for key 'users.email'

-- 기본키(PK)에 중복 값을 입력 시도 (기본키는 중복되면 안됨)
...
-- DEFAULT CURRENT_TIMESTAMP: 새로운 데이터 행이 추가될 때, 해당 컬럼에 별도의 값을 지정하지 않으면 현재 날짜와 시간이 자동으로 입력됨
-- DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP: 새로운 데이터 행이 추가될때는 물론이고,
...
CREATE TABLE capitals (
    ...
    -- country_id 외래키는 countries 테이블의 id컬럼을 가르킨다는 의미(참조)
);
...
-- 참조 무결성 위반 시 테스트 
INSERT INTO capitals (id, name, country_id)
VALUES (104, 'Paris', 999); -- 999는 부모테이블에 없는 id

DELETE FROM countries
WHERE id =1; 
-- 이 나라를 참조하는 수도(자식 테이블)가 있기 때문에 삭제 불가
...
CREATE TABLE players(
    ...
    team_id INT, -- 소속팀 아이디, 일대다 관계에서는 UNIQUE제약 조건 X
	PRIMARY KEY(id),
...
CREATE TABLE settings ( 
    ...
    account_suggestion BOOLEAN NOT NULL,
    user_id INT UNIQUE, -- 1:1연결을 위해 UNIQUE 지정 
    PRIMARY KEY (id), 
    FOREIGN KEY (user_id) REFERENCES users(id)
